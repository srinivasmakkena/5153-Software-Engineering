...
const AddressSelectionPage = ({ customer, setCustomer }) => {
  ...
  const fetchAddresses = async () => {
    try {
      const response = await fetch(`http://localhost:8000/get_address/?customer_id=${customer.id}`);
      if (!response.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8000/add_address/', {
        method: 'POST',
        headers: {
  ...
  const handlePaymentSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8000/add_payment/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
